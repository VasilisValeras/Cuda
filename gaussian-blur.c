...
void gaussian_blur_omp_device(int radius, img_t *imgin, img_t *imgout)
{
    int i, j, k, m;
    int width = imgin->header.width, height = imgin->header.height;
    double weightSum, redSum, greenSum, blueSum;
    ...
    {
        #pragma omp teams num_teams(30) thread_limit(128) 
        #pragma omp distribute parallel for collapse(2) private(k, m, weightSum, redSum, greenSum, blueSum)
        for (i = 0; i < height; i++)
        {
            for (j = 0; j < width; j++)
            {
                weightSum = redSum = greenSum = blueSum = 0.0;
                int tempPos = i * width + j;
                double sigma = radius*radius;

                #pragma omp parallel for reduction(+:weightSum,redSum,greenSum,blueSum) schedule(static,8)
                for (k = i-radius; k <= i + radius; k++)
                {
                    for (m = j-radius; m <= j + radius; m++)
                    {
                        int x = clamp(m, 0, width-1);
                        int y = clamp(k, 0, height-1);
                        int pos = y * width + x;
                        double square = (m-j)*(m-j)+(k-i)*(k-i);
                        double weight = exp(-square / (2*sigma)) / (3.14*2*sigma);

                        redSum += imgin->red[pos] * weight;
                        greenSum += imgin->green[pos] * weight;
                        blueSum += imgin->blue[pos] * weight;
                        weightSum += weight;
                    }
                }
                imgout->red[tempPos] = round(redSum/weightSum);
                imgout->green[tempPos] = round(greenSum/weightSum);
                imgout->blue[tempPos] = round(blueSum/weightSum);
            }
        }
    }
}
...
{
	int i, j, radius;
	double exectime_serial = 0.0, exectime_omp_loops = 0.0, exectime_omp_tasks = 0.0 , exectime_omp_device = 0.0;
	struct timeval start, stop; 
	char *inputfile, *noextfname;   
	char seqoutfile[128], paroutfile_loops[128], paroutfile_tasks[128], paroutfile_device[128];
	img_t imgin, imgout, pimgout_loops, pimgout_tasks, pimgout_device;
